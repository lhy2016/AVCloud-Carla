...
# @bp.route('/property/importAll', methods=['POST'])
# def import_all_properties():
#     payload = request.form.to_dict()
#     properties = json.loads(payload['properties']) 
    
#     pics = {"apartment": os.listdir("hf-pics/apartment"), "townHouse": os.listdir("hf-pics/townHouse"), "detachedSingle": os.listdir("hf-pics/detachedSingle"), "singleFamily": os.listdir("hf-pics/singleFamily")}
#     pointers = {"apartment":0,"townHouse":0, "detachedSingle":0, "singleFamily":0}
#     for property in properties:
#         type = property["property_type"]
#         imgPt = pointers[type]
#         imgName = "hf-pics/" + type + "/" + pics[type][imgPt % len(pics[type])]
#         property_images = []
#         imgs = []
#         with open(imgName,"rb") as image:
#             f = image.read()
#             property_images.append(f)
#         for idx, property_image in enumerate(property_images):
#             resp =  flickr.upload(filename=imgName, format='rest')
#             resp_dict = xmltodict.parse(resp)
#             resp_stats = resp_dict["rsp"]
#             if (resp_stats["@stat"] != "ok"):
#                  continue 
...
#         new_property = Property()
#         new_property.from_dict(property)
#         new_property.from_dict({"imgs": imgs})
#         db.session.add(new_property)
#         db.session.commit()
#         pointers[type] = (imgPt+1)
#     return "finished"
